...
export default function SearchableContent({
  ...
  const [activeVerticals, setActiveVerticals] = useState<Set<string>>(
    new Set(),
  );
  const [showFilters, setShowFilters] = useState(false);
  const [membersWithoutEmbed, setMembersWithoutEmbed] = useState<Set<string>>(
    new Set(),
  );
    ...
    fetch("/api/embedcheck")
      ...
      .then((data) => {
        const failedIds = new Set<string>(
          data.failures?.map((f: EmbedFailure) => f.id) || [],
        );
  ...
  useEffect(() => {
    ...
    setShuffledMembers([
      ...shuffleArray(withEmbed),
      ...shuffleArray(withoutEmbed),
    ]);
      ...
      <div className="content-wrapper">
        <div className="header-section">
          <div className="title-row">
            <h1 className="title">umw.network</h1>
          </div>
          <div className="description">
            <p>
              welcome to the official webring for university of mary washington
              students.
            </p>
            <p>
              our campus is home to builders, writers, artists, researchers,
              organizers, and people doing interesting work across disciplines.
